        ...
        <section className="flex flex-col gap-6" aria-label="회고 뷰">
            ...
            <div className="glass p-5 rounded-xl relative overflow-hidden">
                ...
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-2 bg-white/5-800 p-1 rounded-xl">
                        <button
                            onClick={() => setReviewType('daily')}
                            className={`px-3 py-1.5 rounded-lg text-sm font-bold transition-all ${reviewType === 'daily' ? 'bg-[#111113]-600 shadow text-slate-900 dark:text-white' : 'text-slate-500 hover:text-slate-700'}`}
            ...
            <AnimatePresence mode="wait">
                {isWriting ? (
                    <motion.div
                        ...
                        className="glass p-6 rounded-xl border border-indigo-200/50 dark:border-indigo-500/30"
                    >
                        <div className="mb-6">
                            ...
                            <div className="flex gap-2">
                                {[1, 2, 3, 4, 5].map(s => (
                                    <button
                                        ...
                                        type="button"
                                        onClick={() => setScore(s)}
                                        className={`p-3 rounded-xl text-2xl transition-transform ${score >= s ? 'scale-110 drop-shadow-none' : 'opacity-40 grayscale'}`}
                                        aria-label={`${s}별`}
                                    >
                                        ⭐
                                    </button>
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <textarea
                                    ...
                                    className="w-full bg-[#09090b] border outline-none border-white/10 rounded-xl px-4 py-3 text-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 transition-all min-h-[100px]"
                                />
                            </div>
                            <div>
                                ...
                                <textarea
                                    ...
                                    className="w-full bg-[#09090b] border outline-none border-white/10 rounded-xl px-4 py-3 text-sm focus:border-rose-500 focus:ring-2 focus:ring-rose-500/20 transition-all min-h-[100px]"
                                />
                            </div>
                            <div>
                                <label className="block text-sm font-bold mb-1 text-text-indigo-400">Try (내일 시도할 점)</label>
                                <textarea
                                    value={tryItem}
                                    onChange={(e) => setTryItem(e.target.value)}
                                    placeholder="Problem을 해결하기 위해 내일 당장 실행할 수 있는 작은 행동을 적어보세요."
                                    className="w-full bg-[#09090b] border outline-none border-white/10 rounded-xl px-4 py-3 text-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition-all min-h-[100px]"
                                />
                            </div>
                        </div>
                        ...
                        <div className="flex gap-2 mt-6">
                            <button
                                ...
                                className="flex-1 py-3 bg-white/5-800 text-slate-400 font-bold rounded-xl hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors focus:ring-2 focus:ring-slate-400"
                            >
                                취소
                            </button>
                            <button
                                onClick={handleSave}
                                className="flex-[2] py-3 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-700 transition-colors focus:ring-2 focus:ring-indigo-500 shadow-none shadow-none-500/30"
                            >
                    ...
                    >
                        <div className="flex justify-between items-center mb-6 border-b border-white/10 pb-4">
                            ...
                            <div className="flex gap-2">
                                <button
                                    onClick={handleEdit}
                                    className="px-3 py-1.5 bg-indigo-500/10 text-text-indigo-400 text-sm font-bold rounded-lg hover:bg-indigo-100 transition-colors"
                                >
                                    수정
                                </button>
                                <button
                        ...
                        <div className="space-y-5">
                            ...
                            {currentReview.try && (
                                <div className="bg-indigo-50/50 dark:bg-indigo-500/5 p-4 rounded-xl border border-indigo-100 dark:border-indigo-500/20">
                                    <h4 className="flex items-center gap-1.5 text-sm font-bold text-text-indigo-400 mb-2">
                    ...
                    >
                        <div className="w-16 h-16 bg-white/5-800 rounded-full flex items-center justify-center mb-4 text-3xl shadow-none">
                            ✍️
                        </div>
                        ...
                        <button
                            ...
                            className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 px-6 rounded-full transition-all shadow-none shadow-none-500/30 active:scale-95"
                        >
