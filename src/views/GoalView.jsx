        ...
        <div className="flex-1 min-w-[300px] snap-center flex flex-col gap-3 bg-slate-50/50 dark:bg-white/[0.02] p-4 rounded-3xl border border-slate-100 dark:border-white/5 relative overflow-hidden">
            ...
            {items.length === 0 && (
                <button
                    ...
                    className="w-full py-6 rounded-2xl border-2 border-dashed border-slate-200 dark:border-white/10 hover:border-slate-300 dark:hover:border-white/20 text-slate-400 text-sm font-bold transition-colors flex flex-col items-center justify-center gap-2 mt-2"
    ...
    return (
        <section className="min-h-[600px] mb-8 relative" aria-label="목표 관리">
            <header className="flex justify-between items-center mb-6 pl-2 pr-1">
                <h2 className="text-2xl font-black flex items-center gap-2 text-slate-800 dark:text-slate-100 mt-2">
                    <div className="bg-indigo-100 dark:bg-indigo-500/20 text-indigo-600 dark:text-indigo-400 p-2 rounded-xl" aria-hidden="true">
                ...
                <button
                    ...
                    className="bg-slate-900 dark:bg-indigo-600 text-white px-4 md:px-5 py-2.5 rounded-xl text-sm font-bold transition-all shadow-md flex items-center gap-1.5 hover:bg-slate-800 dark:hover:bg-indigo-500 active:scale-95"
                    ...
                    <div className="fixed inset-0 z-[150] flex items-center justify-center sm:px-4 fade-in" role="dialog" aria-modal="true" aria-labelledby="goal-detail-title">
                        ...
                        >
                            <div className={`h-40 sm:h-48 bg-gradient-to-r ${selectedGoal.colorFrom || 'from-slate-400'} ${selectedGoal.colorTo || 'to-slate-600'} relative shrink-0`} aria-hidden="true">
                                ...
                                <div className="absolute -bottom-10 left-6 sm:left-10 text-6xl bg-transparent drop-shadow-md select-none bg-white/20 p-2 rounded-2xl backdrop-blur-md border border-white/30">{selectedGoal.icon}</div>
                            </div>
                            <div className="px-6 sm:px-10 pt-14 pb-10 flex-1 space-y-6">
                                ...
                                <div className="h-px bg-slate-100 dark:bg-white/5 w-full" role="separator" />

                                {selectedGoal.tracker?.type === 'numeric' ? (
                                    <div className="space-y-3">
                                        <h4 className="text-sm font-black text-slate-800 dark:text-slate-200 flex items-center gap-2"><Target className="w-4 h-4 text-indigo-500" aria-hidden="true" /> 수치 기록 업데이트</h4>
                                        <div className="bg-slate-50 dark:bg-white/5 p-5 rounded-2xl border border-slate-100 dark:border-white/5 space-y-5 shadow-inner">
                                            <div className="flex items-center justify-between">
                                                ...
                                                <div className="text-right"><span className="block text-[11px] font-black text-slate-400 tracking-wider">목표 수치</span><span className="text-lg font-black text-slate-700 dark:text-slate-300">{selectedGoal.tracker.target}</span></div>
                                            </div>
                                            <div className="flex gap-2" role="group" aria-label="수치 조절">
                                                {[-1, 1, 5, 10].map((v) => (
                                                    <button key={v} onClick={() => updateNumericProgress(selectedGoal.id, v)} className="p-3 bg-white dark:bg-[#1a1c23] border border-slate-200 dark:border-white/10 rounded-xl hover:bg-slate-100 dark:hover:bg-white/5 text-slate-600 dark:text-slate-300 font-bold active:scale-95 transition-transform flex-1 shadow-sm">
                                                        {v > 0 ? `+${v}` : v}
                                                    </button>
                                                ))}
                                            </div>
                                    ...
                                    <div className="space-y-3">
                                        <h4 className="text-sm font-black text-slate-800 dark:text-slate-200 flex items-center gap-2"><CheckSquare className="w-4 h-4 text-emerald-500" aria-hidden="true" /> 세부 체크리스트</h4>
                                        {(!selectedGoal.tasks || selectedGoal.tasks.length === 0) && (
                                            <div className="bg-slate-50 dark:bg-white/5 p-4 rounded-2xl border border-slate-100 dark:border-white/5 space-y-2">
                                                ...
                                                <div className="text-right text-xs font-black text-indigo-600 dark:text-indigo-400" aria-live="polite">{selectedGoal.progress}%</div>
                                            </div>
                                        )}
                                        {selectedGoal.tasks && selectedGoal.tasks.length > 0 && (
                                            <div className="space-y-1" role="list" aria-label="체크리스트">
                                                {selectedGoal.tasks.map((t) => (
                                                    <div key={t.id} className="group/task flex items-center justify-between p-2 hover:bg-slate-50 dark:hover:bg-white/5 rounded-xl transition-colors" role="listitem">
                                                        <label className="flex items-start gap-3 flex-1 cursor-pointer">
                                                            <div className="mt-0.5"><input type="checkbox" checked={t.done} onChange={() => toggleTask(selectedGoal, t.id)} className="w-4 h-4 rounded text-indigo-500 focus:ring-indigo-500 bg-white border-slate-300 dark:border-white/20 dark:bg-[#1a1c23] cursor-pointer" /></div>
                                ...
                                <div className="space-y-3 pt-4">
                                    <h4 className="text-sm font-black text-slate-800 dark:text-slate-200 flex items-center gap-2"><List className="w-4 h-4 text-emerald-500" aria-hidden="true" /> 노트 / 기록</h4>
                                    <label htmlFor="goal-memo" className="sr-only">메모</label>
                                    <textarea id="goal-memo" value={selectedGoal.memo || ''} onChange={(e) => updateGoalItem(selectedGoal.id, { memo: e.target.value })} placeholder="목표를 달성하기 위한 구체적인 계획이나 다짐을 자유롭게 기록해보세요." rows={8} className="w-full bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5 rounded-2xl p-4 text-sm resize-none focus:ring-0 focus:outline-none focus:border-slate-300 dark:focus:border-white/20 text-slate-700 dark:text-slate-300 font-medium placeholder:font-medium placeholder:text-slate-400/70" />
                                </div>
                            </div>
                        </motion.div>
                    </div>
            ...
            <AnimatePresence>
                {isCreateModalOpen && (
                    <div className="fixed inset-0 z-[200] flex items-center justify-center px-4" role="dialog" aria-modal="true" aria-labelledby="create-goal-title">
                        ...
                        <motion.div initial={{ scale: 0.95, opacity: 0, y: 10 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 10 }} className="relative glass-card bg-white dark:bg-[#13151a] w-full max-w-sm rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col border border-white/20 dark:border-white/5">
                            <div className="p-7 space-y-6">
                                <div className="flex justify-between items-center pb-2 border-b border-slate-100 dark:border-white/5">
                                    <h3 id="create-goal-title" className="font-black text-xl text-slate-800 dark:text-white flex items-center gap-2">새 페이지 생성</h3>
                                    <button onClick={() => setIsCreateModalOpen(false)} className="bg-slate-100 dark:bg-white/5 p-2 rounded-full hover:bg-slate-200 dark:hover:bg-white/10" aria-label="닫기"><X className="w-5 h-5 text-slate-400" /></button>
                                </div>
                                <div className="space-y-4 max-h-[60vh] overflow-y-auto [&::-webkit-scrollbar]:hidden pb-4">
                                    <div>
                                        ...
                                        <div className="flex gap-2 p-1 bg-slate-100 dark:bg-white/5 rounded-xl" role="radiogroup" aria-label="목표 유형">
                                            {[{ val: 'short', label: '단기' }, { val: 'mid', label: '중기' }, { val: 'long', label: '장기' }].map(({ val, label }) => (
                                                <button key={val} role="radio" aria-checked={goalForm.type === val} onClick={() => setGoalForm({ ...goalForm, type: val })} className={`flex-1 py-2 rounded-lg text-xs font-black transition-all ${goalForm.type === val ? 'bg-white dark:bg-[#1a1c23] text-indigo-600 dark:text-indigo-400 shadow-sm' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-200'}`}>{label}</button>
                                            ))}
                                        </div>
                                    </div>
                                    <div>
                                    ...
                                    <div className="border-t border-b border-slate-100 dark:border-white/5 py-4 my-2">
                                        ...
                                        <div className="flex gap-2 p-1 bg-slate-100 dark:bg-white/5 rounded-xl mb-3" role="radiogroup" aria-label="측정 방식">
                                            ...
                                            <button role="radio" aria-checked={goalForm.trackerType === 'numeric'} onClick={() => setGoalForm({ ...goalForm, trackerType: 'numeric' })} className={`flex-1 py-3 rounded-lg text-xs font-black transition-all ${goalForm.trackerType === 'numeric' ? 'bg-white dark:bg-[#1a1c23] text-indigo-500 shadow-sm border border-slate-200 dark:border-white/5' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-200'}`}>📈 수치 기록 (30+종)</button>
                                        </div>
                                        <AnimatePresence>
                                                ...
                                                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                                                    <div className="grid grid-cols-5 gap-3 bg-slate-50 dark:bg-[#0f1115] p-3 rounded-2xl border border-slate-200 dark:border-white/10 mt-2 shadow-inner">
                                                        <div className="col-span-3">
                                                            ...
                                                            <select id="tracker-unit-select" value={goalForm.trackerUnit} onChange={(e) => setGoalForm({ ...goalForm, trackerUnit: e.target.value })} className="w-full bg-white dark:bg-[#1a1c23] border border-slate-200 dark:border-white/10 rounded-xl px-2 py-2.5 text-xs font-bold text-slate-700 dark:text-slate-200 outline-none cursor-pointer">
                                                        ...
                                                        <div className="col-span-2">
                                                            ...
                                                            <input id="target-value-input" type="number" min="1" value={goalForm.targetValue} onChange={(e) => setGoalForm({ ...goalForm, targetValue: e.target.value })} className="w-full bg-white dark:bg-[#1a1c23] border border-slate-200 dark:border-white/10 rounded-xl px-3 py-2.5 text-xs font-bold text-slate-800 dark:text-slate-200 outline-none focus:border-indigo-500" />
                                                        </div>
                                                    </div>
                                                </motion.div>
                                            )}
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        <div>
                                            ...
                                            <div className="relative group cursor-pointer bg-white dark:bg-[#1a1c23] border border-slate-200 dark:border-white/10 rounded-xl px-4 py-2.5 flex items-center justify-between">
                                                ...
                                                <div className="absolute top-10 left-0 w-[200px] bg-white dark:bg-[#13151a] border border-slate-200 dark:border-white/10 p-2 rounded-xl shadow-xl z-10 hidden group-hover:flex flex-wrap gap-2" role="listbox" aria-label="아이콘 선택">
                                                    {EMOJI_LIST.map((emj) => (<button key={emj} role="option" aria-selected={goalForm.icon === emj} onClick={(e) => { e.preventDefault(); setGoalForm({ ...goalForm, icon: emj }); }} className="w-8 h-8 rounded-lg hover:bg-slate-100 dark:hover:bg-white/10 flex items-center justify-center text-lg">{emj}</button>))}
                                                </div>
                                            </div>
                                        </div>
                                        <div>
                                ...
                                <button onClick={handleCreateGoal} className="w-full bg-slate-900 dark:bg-indigo-600 text-white font-black py-4 rounded-xl shadow-lg mt-2 active:scale-95 transition-transform">생성하기</button>
                            </div>
                        </motion.div>
                    </div>
                )}
