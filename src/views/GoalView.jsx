...
export default function GoalView({ goals, setGoals, studyTimes = {}, studies = [] }) {
    ...
    const renderColumn = (type, label, dotColor, items) => (
        <div className="flex-1 min-w-[300px] snap-center flex flex-col gap-3 bg-slate-50/50 dark:bg-white/[0.02] p-3 rounded-md border border-white/10 relative overflow-hidden">
            ...
            {items.length === 0 && (
                <button
                    ...
                    className="w-full py-3 rounded-md border-2 border-dashed border-white/10 hover:border-slate-300 dark:hover:border-white/20 text-slate-400 text-sm font-bold transition-colors flex flex-col items-center justify-center gap-2 mt-2"
        ...
        <section className="min-h-[600px] mb-8 relative" aria-label="목표 관리">
            <header className="flex justify-between items-center mb-2 pl-2 pr-1">
                ...
                <button
                    ...
                    className="bg-slate-900 dark:bg-indigo-600 text-white px-3 md:px-3 py-2.5 rounded-md text-sm font-bold transition-all shadow-none flex items-center gap-1.5 hover:bg-slate-800 dark:hover:bg-indigo-500 active:scale-95"
            ...
            <div className="glass-card p-3 rounded-md mb-3 flex items-center gap-3 border border-white/10 flex-wrap">
                ...
                {studies.filter(s => s.linkedGoalId).length > 0 && (
                    <div className="flex gap-1.5 flex-wrap ml-auto">
                        {studies.filter(s => s.linkedGoalId).map(s => {
                            ...
                            return (
                                <span key={s.id} className="text-[10px] font-bold bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 px-2 py-0.5 rounded">
            ...
            {(() => {
                ...
                return (
                    <div className="mt-2 bg-amber-500/5 border border-amber-500/20 rounded-md overflow-hidden">
                        <button
                        ...
                        >
                            <div className="flex items-center gap-2">
                                ...
                                <span className="bg-amber-500/20 text-amber-300 text-xs font-bold px-2 py-0.5 rounded">{completedGoals.length}개</span>
                            </div>
                                    ...
                                    <div className="px-3 pb-5 space-y-2.5">
                                        ...
                                        <div className="space-y-2 max-h-64 overflow-y-auto [&::-webkit-scrollbar]:hidden">
                                            {completedGoals.sort((a, b) => (b.completedAt || '').localeCompare(a.completedAt || '')).map(g => (
                                                <div key={g.id} className="flex items-center gap-3 p-3 bg-white/5 rounded-md border border-white/10">
            ...
            {selectedGoal && (
                    <div className="fixed inset-0 z-[150] flex items-center justify-center sm:px-3" role="dialog" aria-modal="true" aria-labelledby="goal-detail-title">
                        ...
                        <div
                            className="relative bg-[#111113] w-full sm:w-[500px] md:w-[650px] h-full sm:h-[85vh] sm:max-h-[800px] sm:rounded-md overflow-y-auto flex flex-col [&::-webkit-scrollbar]:hidden"
                        >
                            <div className={`h-40 sm:h-48  ${selectedGoal.colorFrom || 'from-slate-400'} ${selectedGoal.colorTo || 'to-slate-600'} relative shrink-0`} aria-hidden="true">
                                <button onClick={() => setSelectedGoalId(null)} className="absolute top-4 right-4 bg-black/20 hover:bg-black/40 text-white p-2 rounded-full transition-colors z-10" aria-label="닫기"><X className="w-5 h-5" /></button>
                            ...
                            <div className="px-3 sm:px-10 pt-14 pb-10 flex-1 space-y-3">
                                    ...
                                    {selectedGoal.milestones && selectedGoal.milestones.length > 0 && (
                                        <div className="space-y-1" role="list" aria-label="마일스톤 목록">
                                            {selectedGoal.milestones.map(m => (
                                                <div key={m.id} className="group/ms flex items-center justify-between p-2 hover:bg-white/5 rounded-md transition-colors" role="listitem">
                                ...
                                {selectedGoal.tracker?.type === 'numeric' ? (
                                    <div className="space-y-3">
                                        ...
                                        <div className="bg-[#09090b] p-3 rounded-md border border-white/10 space-y-3 shadow-none">
                                            ...
                                            <div className="flex gap-2" role="group" aria-label="수치 조절">
                                                {[-1, 1, 5, 10].map((v) => (
                                                    <button key={v} onClick={() => updateNumericProgress(selectedGoal.id, v)} className="p-3 bg-[#111113] border border-white/10 rounded-md hover:bg-white/10 text-slate-400 font-bold active:scale-95 transition-transform flex-1 shadow-none">
                                                        {v > 0 ? `+${v}` : v}
                                                    </button>
                                                ))}
                                            </div>
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        {(!selectedGoal.tasks || selectedGoal.tasks.length === 0) && (!selectedGoal.milestones || selectedGoal.milestones.length === 0) && (
                                            <div className="bg-[#09090b] p-3 rounded-md border border-white/10 space-y-2">
                                        ...
                                        {selectedGoal.tasks && selectedGoal.tasks.length > 0 && (
                                            <div className="space-y-1" role="list" aria-label="체크리스트">
                                                {selectedGoal.tasks.map((t) => (
                                                    <div key={t.id} className="group/task flex items-center justify-between p-2 hover:bg-white/10 rounded-md transition-colors" role="listitem">
                                ...
                                <div className="space-y-3 pt-4">
                                    ...
                                    <textarea id="goal-memo" value={selectedGoal.memo || ''} onChange={(e) => updateGoalItem(selectedGoal.id, { memo: e.target.value })} placeholder="목표를 달성하기 위한 구체적인 계획이나 다짐을 자유롭게 기록해보세요." rows={8} className="w-full bg-[#09090b] border border-white/10 rounded-md p-3 text-sm resize-none focus:ring-0 focus:outline-none focus:border-slate-300 dark:focus:border-white/20 text-slate-200 font-medium placeholder:font-medium placeholder:text-slate-400/70" />
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            {isCreateModalOpen && (
                    <div className="fixed inset-0 z-[200] flex items-center justify-center px-3" role="dialog" aria-modal="true" aria-labelledby="create-goal-title">
                        ...
                        <div className="relative glass-card bg-[#111113] w-full max-w-sm rounded-lg overflow-hidden flex flex-col border border-white/20 dark:border-white/5">
                            <div className="p-7 space-y-3">
                                ...
                                <div className="space-y-2.5 max-h-[65vh] overflow-y-auto [&::-webkit-scrollbar]:hidden pb-4">
                                    <div>
                                        ...
                                        <div className="flex gap-2 p-1 bg-white/5 rounded-md" role="radiogroup" aria-label="목표 유형">
                                    ...
                                    <div>
                                        ...
                                        <input id="goal-title-input" autoFocus value={goalForm.title} onChange={(e) => setGoalForm({ ...goalForm, title: e.target.value })} placeholder="예: 토익 900점 완성, 이번주 팀플" className="w-full bg-[#111113] border border-white/10 rounded-md px-3 py-2.5 text-sm font-bold text-slate-400 outline-none focus:border-indigo-500 transition-all" aria-required="true" />
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <div className="flex gap-2 p-1 bg-white/5 rounded-md" role="radiogroup" aria-label="반복 설정">
                                    ...
                                    <div className="border-t border-b border-white/10 py-2.5 my-2">
                                        ...
                                        <div className="flex gap-2 p-1 bg-white/5 rounded-md mb-3" role="radiogroup" aria-label="측정 방식">
                                        ...
                                        {goalForm.trackerType === 'numeric' && (
                                                <div className="overflow-hidden">
                                                    <div className="grid grid-cols-5 gap-3 bg-[#09090b] p-3 rounded-md border border-white/10 mt-2 shadow-none">
                                                        <div className="col-span-3">
                                                            <label htmlFor="tracker-unit-select" className="block text-[10px] font-bold text-slate-500 mb-1.5 pl-1">단위 선택</label>
                                                            <select id="tracker-unit-select" value={goalForm.trackerUnit} onChange={(e) => setGoalForm({ ...goalForm, trackerUnit: e.target.value })} className="w-full bg-[#111113] border border-white/10 rounded-md px-2 py-2.5 text-xs font-bold text-slate-200 outline-none cursor-pointer">
                                                        ...
                                                        <div className="col-span-2">
                                                            ...
                                                            <input id="target-value-input" type="number" min="1" value={goalForm.targetValue} onChange={(e) => setGoalForm({ ...goalForm, targetValue: e.target.value })} className="w-full bg-[#111113] border border-white/10 rounded-md px-3 py-2.5 text-xs font-bold text-slate-400 outline-none focus:border-indigo-500" />
                                                        </div>
                                                    </div>
                                                </div>
                                            )}
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        <div>
                                            ...
                                            <div className="relative group cursor-pointer bg-[#111113] border border-white/10 rounded-md px-3 py-2.5 flex items-center justify-between">
                                                ...
                                                <div className="absolute top-10 left-0 w-[200px] bg-[#111113] border border-white/10 p-2 rounded-md shadow-none z-10 hidden group-hover:flex flex-wrap gap-2" role="listbox" aria-label="아이콘 선택">
                                        ...
                                        <div>
                                            ...
                                            <input id="goal-deadline-input" type="date" value={goalForm.deadline} onChange={(e) => setGoalForm({ ...goalForm, deadline: e.target.value })} className="w-full h-12 bg-[#111113] border border-white/10 rounded-md px-3 py-2 text-xs font-bold text-slate-400 outline-none focus:border-indigo-500 transition-all [color-scheme:light] dark:[color-scheme:dark]" aria-required="true" />
                                        </div>
                                    </div>
                                    <div>
                                ...
                                <button onClick={handleCreateGoal} className="w-full bg-slate-900 dark:bg-indigo-600 text-white font-bold tracking-tight py-2.5 rounded-md shadow-none mt-2 active:scale-95 transition-transform">생성하기</button>
                            </div>
                        </div>
                    </div>
                )}
